...
describe("pi-specdocs runtime", () => {
	...
	it("runs specdocs-validate and reports errors/warnings", async () => {
		...
		writeFileSync(join(base, "docs", "prd", "PRD-001-good.md"), '---\nprd: PRD-001\ntitle: "Good"\nstatus: Draft\nowner: Alice\ndate: 2025-01-01\nissue: 1\nversion: 1\n---\n');
		writeFileSync(join(base, "docs", "prd", "PRD-003-gap.md"), '---\nprd: PRD-003\ntitle: "Gap"\nstatus: Draft\nowner: Bob\ndate: 2025-01-01\nissue: 2\nversion: 1\n---\n');
		writeFileSync(join(base, "docs", "prd", "bad-name.md"), "# invalid filename\n");
		writeFileSync(join(base, "docs", "adr", "ADR-0002-gap.md"), '---\nadr: ADR-0002\ntitle: "Gap ADR"\nstatus: Proposed\ndate: 2025-01-01\nprd: PRD-001\n---\n');
		...
		expect(notify).toHaveBeenCalledWith(expect.stringContaining("filename doesn't match PRD-NNN-*.md pattern"), "error");
	});
});
