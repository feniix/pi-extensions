...
| Company-profile style results | `web_search_advanced_exa` with `category: "company"` | Use when available; keep synthesis in `web_research_exa`. |
...
- `web_search_advanced_exa` owns filtered retrieval; `web_research_exa` owns Agent-based synthesis.
