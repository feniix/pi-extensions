...
import type { AuthResolution, ExaConfig } from "./types.js";
import { getHomeDir, normalizeString } from "./types.js";
...
function loadSettingsConfig(path: string): ExaConfig | null {
  ...
  try {
    ...
    // Only return config if it has meaningful settings
    if (!parsedConfig.enabledTools?.length && !parsedConfig.advancedEnabled) {
      return null;
    }
    return {
...
export function getConfigOverrideFlag(pi: ExtensionAPI): string | undefined {
...
export function isAdvancedToolEnabled(pi: ExtensionAPI, config: ExaConfig | null): boolean {
