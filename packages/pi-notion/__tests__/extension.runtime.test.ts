...
describe("pi-notion extension runtime", () => {
	it("warns for incorrect notion-search inputs", async () => {
		...
		await toolCall?.(
			{ toolName: "mcp__notion-search", input: { query: "meeting notes" } },
			{ ui: { notify } },
		);

		expect(notify).toHaveBeenCalledWith(expect.stringContaining("content_search_mode is not 'workspace_search'"), "warning");
	...
	it("warns for raw fetch ids and empty meeting note filters", async () => {
		...
		await toolCall?.(
			{ toolName: "mcp__notion-query-meeting-notes", input: { filter: {} } },
			{ ui: { notify } },
		);
